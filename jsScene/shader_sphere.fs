 precision highp float;
  ...
  void main() {
    ...
    vec4 exp = vec4(rgb * vec3(1.0 - bright + 0.6) + whiteNoise * 0.2, 0.4 + vOpacity * 0.5 + sin(time * 2.0) * 0.1); 
	gl_FragColor = vec4(exp.x*1.0+0.1, exp.y-(exp.x*0.5), exp.z+(exp.y*0.5), exp.a);
